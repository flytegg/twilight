...
fun main() {
    ...
    // POJO test
    assert(person.toInsertQuery() == "INSERT INTO people (age, id, name) VALUES ('13', '1', 'Test')") { "SQL INSERT Query did not match" }
    //
    ...
    */
    ...
    assert(database.execute(person.toInsertQuery())) { "Failed to insert a person into the table!" }

    // getting POJO's

    val people = database.executeQuery(QueryBuilder().select("*").from("people").where("id" eq 1).build())?.toListOfObjects<Person>()
